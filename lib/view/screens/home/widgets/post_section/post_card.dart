...
import '../comon_post_action_sheet.dart';
...
class _PostCardState extends State<PostCard> {
  ...
  bool _isImageLoading = true; // New: Track image loading state
  ...
  Widget _buildMediaContent() {
    ...
    if (widget.item.videoUrl != null && widget.item.videoUrl!.isNotEmpty) {
      return ClipRRect(
        ...
        widget.item.postImage!.isNotEmpty) {
      // Image content with shimmer when loading
      if (_isImageLoading) {
        return Shimmer.fromColors(
          ...
          child: Container(
            width: double.infinity,
            height: calculatedHeight, // Use calculated height for image shimmer
            color: Colors.white,
          ),
        );
      } else {
        return ClipRRect(
          ...
          child: CustomNetworkImage(
            imageUrl: widget.item.postImage!,
            fit: BoxFit.cover,
            width: double.infinity,
            height: calculatedHeight, // Use calculated height for the image
          ),
        );
      }
    } else {
      // If neither video nor image is provided, return an empty space
        ...
        children: [
          Column(
            ...
            children: [
                    ...
                    GestureDetector(
                      onTap: () {
                        showModalBottomSheet(
                          context: context,
                          isScrollControlled:
                              true, // This is important for DraggableScrollableSheet
                          backgroundColor: Colors
                              .transparent, // Often useful for draggable sheets
                          builder: (BuildContext context) {
                            return const ComonPostActionSheet(); // Correctly instantiate your widget
                          },
                        );
                      },
                      child: const Icon(
              ...
              _buildMediaContent(), // This will now correctly show shimmer or nothing
